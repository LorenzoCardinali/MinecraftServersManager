...
if ! fn_is_present "$SERVER_PATH" || ! fn_is_present "$DATA_FOLDER"; then
    fn_error "Server [$SERVER_NAME] doesn't exist."
fi
...
# log file check
if ! fn_is_present "$LOG_FILE"; then
...
function fn_eula_check() {
    if fn_is_present "$EULA_FILE"; then
        if grep -q "false" "$EULA_FILE"; then
            fn_eula_agree
        fi
    else
...
function fn_eula_agree() {
    ...
    if fn_prompt_yn "Do you agree?" Y; then
        printf "#%s \neula=true\n" "$(date)" >"$EULA_FILE"
    else
...
start)
    if fn_session_check; then
        ...
        echo "$SESSION_NAME"
        echo "$SERVER_NAME"
        tmux new-session -s "${SESSION_NAME}" #"bash \"$MODULE_PATH/$START_MODULE\""
        #-d
        #tmux set-environment -t "${SESSION_NAME}" SERVER_NAME "$SERVER_NAME"
        #tmux set-environment -t "${SESSION_NAME}" SERVER_PATH "$SERVER_PATH"
        #tmux set-environment -t "${SESSION_NAME}" JAR_FILE "$JAR_FILE"
        #tmux set-environment -t "${SESSION_NAME}" CONFIG_FILE "$CONFIG_FILE"
        #tmux set-environment -t "${SESSION_NAME}" LOG_FILE "$LOG_FILE"
        #tmux set-environment -t "${SESSION_NAME}" STATUS_FILE "$STATUS_FILE"
        tmux send-keys -t "${SESSION_NAME}" "bash \"$MODULE_PATH/$START_MODULE\"" Enter
    fi
    ;;
...
status)
    if fn_is_present "${STATUS_FILE}"; then
        cat "$STATUS_FILE"
    else
